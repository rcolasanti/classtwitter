...
#
`#  
#  Program description to be added here:
#
...
from tkinter import *
# The twitter accout information
...
class TkinterGraph(Frame):
    ...
    def add_text(self,text,time):
        # Display answer in ScrolledText
...
class TkTwitter:
    def __init__(self,question,results_file):
        ...
        # Run get tweet task after 2 seconds
    ...
    def save_tweets(self):
        # Save the list of answers to the file
    ...
    def task(self):
        ...
        for tweet in search_results:
            ...
            if not tweet.id in self.results:
                # Only show tweets that have been sent after the 
                # program has started
                # DEBUG print(tweet.created_at+ datetime.timedelta(hours=1),self.time_start)
                # Extra hour added for DST 
                #       . Must make this automatic or stre time zone
...
def main():
    print (len(sys.argv))
    if len(sys.argv)==3:
        # Get the question [1] amd the name of the file where the
        # question answers [2] will be saved
